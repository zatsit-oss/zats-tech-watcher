...
export function renderCardItem(entry: TechWatchEntry): string {
  return `
    <article class="group relative glass rounded-xl p-5 transition-all duration-200 glass-hover flex flex-col gap-3">
      ...
      <h3 class="text-sm font-semibold leading-snug line-clamp-2 text-content">
        <a href="${escapeHtml(entry.url)}" target="_blank" rel="noopener noreferrer"
           aria-label="${escapeHtml(entry.subject)} (nouvel onglet)"
           class="transition-colors group-hover:text-primary after:content-[''] after:absolute after:inset-0 after:rounded-xl">
      ...
      <div class="relative z-10 mt-auto flex items-center justify-between pt-2 border-t border-edge pointer-events-none">
        <span class="pointer-events-auto inline-flex">${renderContributorName(entry.contributor, "text-xs font-medium")}</span>
        <span class="text-xs text-content-muted group-hover:text-primary transition-colors flex items-center gap-1" aria-hidden="true">
          Ouvrir
